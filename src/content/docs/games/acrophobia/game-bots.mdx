...
graph TD
    ...
    E[Game List IRC Server]
    F[Game Room IRC Server]

    A --> B
    B -->|serves Dispatch.ini HTTP| C
    C -->|serves UpdateScript.ini + assets HTTP| D
    D -->|/cgi/acrval0.cgi HTTP CGI via Acrobot| E
    E -->|#Acro_List channel Acrobot IRC bot| F

    F -.->|#Acro_000, etc. Acrobot IRC bot| F

    style A fill:#e066ff,stroke:#333,color:#fff
    style D fill:#9999ff,stroke:#333,color:#fff
```
---
